...
namespace Ai.Hgb.Runtime {
  ...
  public class Repl {
    ...
    #region fields
    private static string[] COMMANDS = { "run", "demo", "state", "startup", "pause", "stop", "attach", "detach", "cancel", "save", "exit", "q", "quit", "list", "monitor", "generate", "stubs", "desc", "build", "deploy" };
    ...
    private List<Generator.ProjectInfo> projects;
    #endregion fields
      ...
      foreach (var cmd in commands) {
        if (cmd.Key == "exit" || cmd.Key == "q" || cmd.Key == "quit") {
          ...
          if (cmd.Value != null && cmd.Value.Count > 0) {
            ...
            if (!foundFile) path = Path.Combine(@"..\..\..\..\DemoApps\Texts\", path);
            if (File.Exists(path)) foundFile = true;
          }
          ...
          if (cmd.Value != null && cmd.Value.Count == 2) {
            descriptionPath = cmd.Value[0];
            if (!descriptionPath.EndsWith(".3l")) descriptionPath = descriptionPath + ".3l";
            if (File.Exists(descriptionPath)) foundFile = true;
            if (!foundFile) descriptionPath = Path.Combine(@"..\..\..\..\DemoApps\Texts\", descriptionPath);
            if (File.Exists(descriptionPath)) foundFile = true;
            
            string stubsPath = cmd.Value[1];
            this.projects = GenerateSolution(descriptionPath, stubsPath);
          }
        }
        else if (cmd.Key == "build") {
          Console.WriteLine("building...");

          
          
        }
        else if (cmd.Key == "deploy") {
    ...
    private List<Generator.ProjectInfo> GenerateSolution(string sourcePath, string targetDir) {
      sst = ReadSeidl(sourcePath);
      var generator = new Generator();
      var projects = generator.GenerateSolution(sst, targetDir, true);
      return projects;
    ...
    private ScopedSymbolTable ReadSeidl(string path) {
      ...
      var transformer = new Transformer(parser);
      transformer.ProgramTextUrl = path;

      return transformer.CreateScopedSymbolTableSecured();
    }
